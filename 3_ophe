...
class TemplateManager:
    ...
    def food_prompt(self, field_values, color_scheme):
        caption = (
            f"🍴 Discover our delicious {field_values['recipe_name']} recipe! "
    ...
    def ad_prompt(self, field_values, color_scheme):
        caption = (
            f"✨ Introducing our new {field_values['product_name']} {field_values['product_type']}! "
    ...
    def book_prompt(self, field_values, color_scheme):
        caption = (
            f"📚 New Release: {field_values['book_title']} by {field_values['author_name']}! "
...
class AuthSystem:
    ...
    def show_signup_form(self):
        return widgets.VBox([
            widgets.HTML("<h2 style='color:#2E86e1; margin-top:0'>📝 Create Account</h2>"),
...
content_generator_tab = widgets.VBox([
    widgets.HTML("<h2 style='color:#2E86e1; margin-top:0'>📱 Industry-Specific Content Generator</h2>"),
    ...
    widgets.Accordion(children=[caption_output, prompt_output],
                     titles=('📝 Social Media Caption', '🎨 Image Prompt')),
...
def generate_images():
    ...
    if not template_name:
        with status_output:
            display(HTML(
                ...
                "⚠️ Please select a template first!</div>"
            ))
        return
    ...
    if not hasattr(dynamic_form_output, 'field_widgets'):
        with status_output:
            display(HTML(
                "<div style='background:#ffebee; padding:15px; border-radius:5px; color:#c62828'>"
                "⚠️ Please wait for form to load!</div>"
            ))
        return
        ...
        if not field_values[name]:
            with status_output:
                display(HTML(
                    ...
                    f"⚠️ Please fill in all fields! Missing: {name}</div>"
                ))
            return
        ...
        for i in range(2):
            ...
            if image_bytes is None:
                img_widget = widgets.HTML(
                    ...
                    f"❌ Image failed to generate. Please try again.</div>"
                )
            else:
                # Create image widget
    ...
    if template_manager.regeneration_count >= template_manager.max_regenerations:
        with feedback_output:
            display(HTML(
                ...
                "⚠️ Maximum regenerations reached (20 attempts). Please start a new session.</div>"
            ))
        return
